...
import com.tactorder.gateway.api.EmbeddingsHandler
import com.tactorder.gateway.api.ModelsHandler
...
class GatewayVerticle : CoroutineVerticle() {
    ...
    override suspend fun start() {
        ...
        // API Routes
        val modelsHandler = ModelsHandler(vertx)
        router.get("/v1/models").handler(modelsHandler::handle)

        val chatHandler = ChatCompletionHandler(vertx)
        router.post("/v1/chat/completions").handler(chatHandler::handle)
        
        val embeddingsHandler = EmbeddingsHandler(vertx)
